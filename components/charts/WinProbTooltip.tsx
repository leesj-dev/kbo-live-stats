...
function resultText(g: WpGame): string {
  const label = OUTCOME_LABEL[wpOutcome(g.wpClose)];
...
function Sparkline({
  ...
  const endColor = gained ? WIN_COLOR : LOSS_COLOR;
...
export function WinProbTooltip({
  c,
  team,
  color,
  point,
      ...
      <div className="mb-0.5 flex items-center justify-between gap-4">
        ...
        {games.length === 1 && (
          <span
            ...
            style={{ color: OUTCOME_COLOR[wpOutcome(games[0].wpClose)] }}
          >
            {resultText(games[0])}
          </span>
        )}
        ...
        >
          {games.length > 1 && (
            <div className="mb-0.5 flex items-center justify-between gap-6">
              ...
              <span
                className="text-[14px] font-bold"
                style={{ color: OUTCOME_COLOR[wpOutcome(g.wpClose)] }}
              >
                {resultText(g)}
              </span>
            </div>
